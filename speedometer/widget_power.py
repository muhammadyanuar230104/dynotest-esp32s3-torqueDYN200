...
from PySide6.QtGui import QPainter, QPen, QFont, QColor, QBrush, QLinearGradient, QRadialGradient, QConicalGradient
...
class PowerWidget(QWidget):
    ...
    def setValue(self, val):
        # Hitung skala dinamis (_max_value dibulatkan ke atas berdasarkan nilai masuk)
        if val > self._max_value:
            # Naikkan ke kelipatan terdekat dari 10, 100, 1000, dst.
    ...
    def paintEvent(self, event):
        ...
        danger_threshold = int(self._max_value * 0.8)  # 75% dari max dianggap zona merah
        color = QColor("#FF0000") if self._display_value >= danger_threshold else QColor("#FFFFFF")
        start_danger = self.valueToAngle(danger_threshold)
        ...
        painter.setPen(QPen(QColor("#FF0000"), 8))  # Merah terang
        ...
        #ukuran kotak lebih ramping 
        box_width = rect.width() * 0.35
        box_height = 38
        box_x = rect.center().x() - box_width / 2
        box_y = rect.bottom() - box_height - 14
        display_rect = QRectF(box_x, box_y, box_width, box_height)
        #background grafient box
        ...
        label_text = "Power (W)"
        label_rect = QRectF(
            box_x,      #geset ke kiri sedikit
            box_y - 24, #naik sedikit biar pas jaraknya
            box_width + 10, #tambah lebar kiri kanan 
            20          #tinggi label sedikit lebih tinggi
        )
        #shadow layer 
    ...
    def drawHighlightArc(self, painter, rect, radius):
        ...
        if self._display_value <= danger_threshold:
            ...
            painter.setPen(QPen(QColor("#32CD32"), 8))  # Hijau
            painter.setBrush(Qt.NoBrush)
            ...
            # Arc merah dari danger ke current
            red_span = angle_current - angle_danger
            painter.setPen(QPen(QColor("#FF0000"), 8))  # Merah
