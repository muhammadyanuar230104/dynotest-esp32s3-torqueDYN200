...
from PySide6.QtGui import QPainter, QPen, QFont, QColor, QBrush, QRadialGradient, QLinearGradient, QConicalGradient
from PySide6.QtCore import Qt, QRectF, QTimer, QPointF, QPropertyAnimation, Property, QSize
import math
class TorqueGaugeWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._theme_color = "biru"
        self._value = 0
        self._display_value = 0
        self._max_value = 10  # Torque range 0-200
        self._min_angle = 225
        self._max_angle = -45
        # Smooth animation using QPropertyAnimation
        self._anim = QPropertyAnimation(self, b"displayValue")
        self._anim.setDuration(300)  # Smooth animation duration
    ...
    def setValue(self, val):
        #val = torque_cal yang akan ditampilkan & dipakai skala
    ...
    def paintEvent(self, event):
        ...
        for i in range(steps + 1):
            ...
            #ganti warna label sesuai nilai
            if i < steps:
        ...
        #draw red danger zone from 160 to 200
        ...
        # Highlight arc for needle movement
        ...
        # Value label (digital display) below the circle
        ...
        painter.drawText(display_rect, Qt.AlignCenter, f"{self._display_value:.2f}")
    def setTorqueCal(self, torque_cal):
        if torque_cal > self._max_value:
            magnitude = 10 ** int(math.log10(torque_cal))
            self._max_value = math.ceil(torque_cal / magnitude) * magnitude
            self.update()
